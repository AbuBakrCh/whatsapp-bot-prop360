...
import {
  getIncompleteTimetables,
  getIncompleteTimetablesForUser,
} from "../api";
...
export default function IncompleteTimetables() {
  ...
  const fetchPage = useCallback(
    async (pageNumber = 1, query = "") => {
      ...
      const res = await getIncompleteTimetables({
        period,
        page: pageNumber,
        pageSize: LIMIT,
        q: query,
  ...
  const openUserModal = async (row) => {
    ...
    const res = await getIncompleteTimetablesForUser(row.userId, period);
    setModalLoading(false);

    if (res.error) {
      setModalError(
        typeof res.error === "string" ? res.error : "Failed to load activities"
      );
      return;
    }
    ...
    setModalActivities(res.activities || []);
  ...
  return (
    <div className="max-w-4xl mx-auto mt-6 bg-white rounded-2xl shadow-lg p-6 border border-slate-200">
      ...
      {isLoading ? (
        ...
        <p className="text-gray-500">No incomplete timetables found.</p>
      ) : (
                ...
                <div className="space-y-3">
                  {modalActivities.map((activity) => (
                    ...
                    >
                      <div className="flex flex-wrap items-center gap-2 mb-2">
                        ...
                        {activity.missingContact && (
                          <span className="text-xs px-2 py-0.5 rounded bg-amber-100 text-amber-800">
                            No contact
                          </span>
                        )}
                        {activity.missingProperty && (
                  ...
                  {modalTruncated && (
                    <p className="text-xs text-amber-700">
                      Showing first 500 activities only.
                    </p>
                  )}
                </div>
              )}
