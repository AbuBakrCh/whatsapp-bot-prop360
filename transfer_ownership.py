...
async def transfer_ownership(prop_db):
    ...
    - Docs starting with api-token-: existing merchantId → sharedWithMerchants,
      merchantId → Kostas; owner is not changed
    - API-token transfer only when both spitogatos_url and spitogatos are null
    """
    ...
    # --------------------------------------------------
    # Step 1a: API-token docs — share previous merchant, set Kostas as merchantId
    # --------------------------------------------------
    api_docs_result = await formdatas_col.update_many(
        {
            ...
            "merchantId": {"$ne": KOSTAS_MERCHANT_ID},
            "$and": [
                {"spitogatos_url": None},
                {"spitogatos": None},
            ],
